...
```{r}
setwd("C:\\Users\\ito\\Desktop\\UNI\\Working Directory\\3rd semester\\Assignment-1")
...
```{r}
data_demo=read.csv("C:\\Users\\ito\\Desktop\\UNI\\Working Directory\\3rd semester\\Assignment-1\\demo_train(1).csv")

data_LU=read.csv("C:\\Users\\ito\\Desktop\\UNI\\Working Directory\\3rd semester\\Assignment-1\\LU_train.csv")

data_token=read.csv("C:\\Users\\ito\\Desktop\\UNI\\Working Directory\\3rd semester\\Assignment-1\\token_train.csv")
```
...
```{r}
#rename(df, x=y), renames y to x
data_demo=dplyr::rename(data_demo, SUBJ=Child.ID)
data_demo=dplyr::rename(data_demo, VISIT=Visit)
data_demo=dplyr::rename(data_demo, nonVerbalIQ=MullenRaw)
data_demo=dplyr::rename(data_demo, verbalIQ=ExpressiveLangRaw)
...
```{r}
data_demo$VISIT=stringr::str_extract(data_demo$VISIT,"\\d")
data_LU$VISIT=stringr::str_extract(data_LU$VISIT,"\\d")
data_token$VISIT=stringr::str_extract(data_token$VISIT,"\\d")
...
```{r}
#gsub("x","y",df) replaces x with y in the dataframe. \\. is a general expression for punctuation
...
demo_sub=select_(data_demo,"SUBJ","VISIT","Ethnicity","Diagnosis","Gender","Age","ADOS","nonVerbalIQ","verbalIQ")

LU_sub=select_(data_LU, "SUBJ", "VISIT", "MOT_MLU", "MOT_LUstd", "CHI_MLU", "CHI_LUstd")

token_sub=select_(data_token, "SUBJ", "VISIT", "types_MOT", "types_CHI", "tokens_MOT", "tokens_CHI")
...
```{r}
datamerge1 = merge(demo_sub,LU_sub,by= c("SUBJ", "VISIT"))
datamerge2 = merge(datamerge1, token_sub, by=c("SUBJ", "VISIT"))
```
...
datamerge3$Gender = mapvalues(datamerge3$Gender, c("1", "2"),c("F", "M"))
...
```{r}
write.csv(datamerge3, file = "finaldata.csv", row.names = FALSE)
```
...
```{r}
#1 select not strictly necessary
filter_1 = select_(datamerge3,"SUBJ","Visit","CHI_MLU") %>%
  filter(CHI_MLU > 2.7)

unique(filter_1$SUBJ)

#2
filter_2 = select_(datamerge3,"SUBJ","Visit","CHI_MLU") %>% 
  filter(Visit == 1, CHI_MLU < 1.5)

unique(filter_2$SUBJ)

#3
filter_3=select_(datamerge3,"SUBJ","Visit")%>%
  group_by(SUBJ) %>%
  filter(max(Visit) < 6)

unique(filter_3$SUBJ)
...
```{r}
#1
select_(datamerge3,"SUBJ","Visit","CHI_MLU") %>%
  filter(Visit == 6) %>%
...
```{r}
#1
ASD_data=select_(datamerge3,"SUBJ","Diagnosis","CHI_MLU","tokens_CHI") %>%
  filter(Diagnosis=="ASD")


#2
select_data2 = datamerge3 %>%
  filter(Diagnosis == "ASD")%>%
  select(Diagnosis, Diagnosis,CHI_MLU, tokens_CHI)
...
```{r}
#1
datamerge3 = mutate(datamerge3, GrandAvg = mean(tokens_CHI))

#2
ChiAvg_AcrossVisits = select_(datamerge3, "SUBJ", "tokens_CHI") %>%
  group_by(SUBJ)%>%
  summarise(ChiAvgWord = mean(tokens_CHI)) %>%
  merge(datamerge3)

#3 when we're interested in development, the mean doesn't tell us much
